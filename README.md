...
- **تحليل بصمة جغرافية لحسابات تويتر** عبر أداة GeoSocial Footprint (اختياري).
...
- Python 3.10 (أو 3.11) ويفضَّل 3.10 لضمان توفر عجلات `dlib` الجاهزة
  وتجنب مشاكل التثبيت البطيء مع الإصدارات الأحدث.
- Node.js 18 أو أحدث.
- الحزم الموجودة في `backend/requirements.txt` و`frontend/package.json`.
- تثبيت أداة Maigret بشكل مستقل.
- تثبيت أداة GeoSocial Footprint يدويًا في حال رغبت باستخدام ميزة البصمة الجغرافية.
...
يمكن التحقق من توفر الحزم الثقيلة عبر زيارة المسار `/api/health` الذي يعرض
حالة تحميلها عند بدء التطبيق.
